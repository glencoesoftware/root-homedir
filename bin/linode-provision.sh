...
(
# <udf name="u_hostname" label="Hostname:" example="coolservername.glencoesoftware.com">
# <udf name="puppet_env" label="Puppet environment?" oneof="client,master,masterless,none" default="masterless">
# <udf name="puppet_master" label="Puppet Master IP" default="" example="Leave blank for self-master">
# <udf name="puppet_roles" label="Puppet roles (space delimited):" default="" example="role:ldap_server[eq]true">
...
function bootstrap_rpm() {
  ...
  # root homedirectory management
  # tar  up existing homedir
  tar cvfz /tmp/root-homedir.tar.gz /root/
  # remove existing dir
  rm -rvf /root/
  # clone from repo
  git clone git://github.com/glencoesoftware/root-homedir.git /root/
  # restore files, they will be unrevisioned but we wont lose anything
  ( cd / && tar xvfz /tmp/root-homedir.tar.gz )
...
[[ -z "$U_HOSTNAME" ]] && echo "Missing U_HOSTNAME" && DIE=$(( $DIE + 1 ))
